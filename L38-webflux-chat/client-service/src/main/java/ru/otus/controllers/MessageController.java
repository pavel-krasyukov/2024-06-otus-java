...
public class MessageController {
    ...
    public void handleSessionSubscribeEvent(SessionSubscribeEvent event) {
        ...
        //Здесь получаем сообщения для конкретной комнаты
    ...
    private Flux<Message> getMessagesByRoomId(long roomId) {
        if (roomId == ROOM_1408) {
            return datastoreClient.get().uri("/msg").accept(MediaType.APPLICATION_NDJSON).exchangeToFlux(response -> {
